...
OBSDataAutoRelease sourceState(obs_source_t *source, obs_data_t *settings)
{
	...
	if (auto *parent = obs_filter_get_parent(source))
		obs_data_set_string(state, "parent", obs_source_get_uuid(parent));
...
void addSourceUndo(obs_source_t *source, obs_data_t *before, obs_data_t *after, const QString &label)
{
	...
	obs_frontend_add_undo_redo_action(name.constData(), restoreSource, restoreSource, oldJson.constData(),
					  newJson.constData(), false);
}
...
void inspector::updateSource(obs_source_t *source, obs_data_t *before, obs_data_t *after)
{
	...
	addSourceUndo(source, oldState, newState, text("Undo.Properties"));
	obs_frontend_save();
}
...
void inspector::setFilterEnabled(obs_source_t *filter, bool enabled)
{
	...
	addSourceUndo(filter, oldState, newState, text("Undo.Filter"));
	obs_frontend_save();
}
			      ...
			      const std::function<void()> &edit)
{
	...
	if (oldJson == newJson)
		return;
	const auto name =
		(label + " — " + QString::fromUtf8(obs_source_get_name(obs_sceneitem_get_source(item)))).toUtf8();
	obs_frontend_add_undo_redo_action(name.constData(), restoreTransform, restoreTransform, oldJson.constData(),
					  newJson.constData(), false);
	obs_frontend_save();
}
